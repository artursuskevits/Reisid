		...
		<html>
			...
			<body>
				...
				<xsl:for-each select="reisid/reis/details[substring(kuupaev, 1,4)=2024]">
					<xsl:sort select="number(substring(hind, 2,4))+100" order="ascending"/>
					<h1>
					...
					<ul>
						...
						<xsl:choose>	
						<xsl:when test=" number(substring(hind,2,3))  >= 100">
							<li class="expensive">
								Ei ole econom reis
							</li>
						</xsl:when>
						...
						<li>
							Kõik hind on <xsl:value-of select="number(substring(hind,2,5)) + 150"/>
						</li>
					</ul>
				</xsl:for-each>
				<table>
					...
					<tbody>
						<xsl:for-each select="reisid/reis/details">
							<tr>
								...
								<td>
									<xsl:choose>
										<xsl:when test="number(substring(hind, 2, string-length(hind))) >= 100">
